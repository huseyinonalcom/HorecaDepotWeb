...
export const getTopProductsWeek = async ({
  ...
  try {
    if (!date) {
      date = subtractDays(new Date(), 7).toISOString().split("T")[0]; // Default to today if no date is provided
    }

    const request = await fetch(
      `${apiUrl}/api/product-view-stats?sort=times_viewed:desc&pagination[pageSize]=${count}&filters[statistic][date][$gte]=${date}&populate[product][populate][0]=categories&populate[product][populate][1]=images`,
      {
        headers: {
          Authorization: `Bearer ${process.env.API_KEY}`,
        },
