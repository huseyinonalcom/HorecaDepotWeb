    ...
    if (
      ...
      const post = async () => {
        const request = await fetch(
          "/api/client/client/postnewaddress?client=" +
            loggedClient.client_info.id,
          {
            method: "POST",
            body: JSON.stringify({
        ...
        if (request.ok) {
          ...
          setLoggedClient(answerUpdatedClient);
        }
      };
      post();
    }
    ...
    <Layout>
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        <div className="order-2 mb-2 mt-1 p-3 shadow-lg sm:order-1">
          {loggedClient && (
            <div className="flex flex-col">
                ...
                <div className="flex flex-col">
                  <p>
                    ...
                    <span className="font-bold">
                      {loggedClient.client_info.firstName}{" "}
                      {loggedClient.client_info.lastName}
                    </span>
                    <br />
                    {t("Phone")}: {loggedClient.client_info.phone} <br />
                    {t("Email")}: {loggedClient.email}
                  </p>
                </div>
                <div>
                  <p className="">
                    {t("wrong_user", {
                      userName: `${loggedClient.client_info.firstName} ${loggedClient.client_info.lastName}`,
                    })}
                  </p>
                  <button
              ...
              {loggedClient.client_info.addresses &&
                loggedClient.client_info.addresses.length > 0 && (
                  <div>
                    <div className="mt-2">
          ...
          {!loggedClient && !showLoginForm && !showRegisterForm && (
