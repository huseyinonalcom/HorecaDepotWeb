...
<g:title>${sanitizeXml(prd.categories.at(0).localized_name.fr + " " + (prd.localized_name ? prd.localized_name.fr : prd.name))}</g:title>
<g:description>${sanitizeXml(prd.localized_name ? prd.localized_name.fr : prd.name) + " " + sanitizeXml(prd.localized_description?.fr ?? prd.categories.map((c) => c.localized_name.fr).join(", "))}</g:description>
